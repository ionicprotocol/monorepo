...
export class DiscordService {
    ...
    private failureHook = new Webhook("https://discord.com/api/webhooks/1037996046623723541/PBt_TCSJUFowrgupBvH0djFUv5XH3OB_q_4LLZFcV86vFuWnDzYWHOXanwtwS1yu2tpC");
    private successHook = new Webhook("https://discord.com/api/webhooks/1291393275621801984/KnxzuJy1yrJeX6GzJO94Zsr7j9Uolsu2FYNj5K4JZLADzn9Wq-Kg7kIpw4P4b2ICWTKL");
    ...
    private async sendWithRetry(hook: Webhook, message: MessageBuilder, retries: number = 3): Promise<void> {
        ...
        while (attempt < retries) {
            try {
                ...
                if (error.response?.statusCode === 429) {  // Rate limit error
    ...
    async sendLiquidationFailure(pool: LiquidatablePool, errorMessage: string): Promise<void> {
        ...
        baseMessage.setDescription(`Failed Liquidation:\nBorrower: ${pool.liquidations[0].borrower}\nError: ${errorMessage}`);
