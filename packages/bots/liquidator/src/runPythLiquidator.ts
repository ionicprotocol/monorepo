...
async function calculateTotalValueInEth(
    ...
    // Placeholder: Replace with the actual token price fetching logic
    const tokenPriceInEth = BigInt(1e18); // Example: 1 ETH per token
    // Calculate total value in ETH
    const totalValueInEth = tokenPriceInEth * tokenAmount / BigInt(1e18);
...
(async function () {
    ...
    for (const liquidatablePool of liquidatablePools) {
        logger.info(`Liquidating pool: ${liquidatablePool.comptroller} -- ${liquidatablePool.liquidations.length} liquidations found`);
        ...
        for (const liquidation of liquidatablePool.liquidations) {
            ...
            if (liquidation.buyTokenUnderlying && liquidation.sellTokenUnderlying) {
                logger.info(`Buying Token (Underlying): ${liquidation.buyTokenUnderlying}, Amount: ${liquidation.buyTokenAmount.toString()}`);
                logger.info(`Selling Token (Underlying): ${liquidation.sellTokenUnderlying}, Amount: ${liquidation.sellTokenAmount.toString()}`);
