      ...
      incentivesForCToken.length && !!cTokenData
        ? incentivesForCToken.map((incentiveForCToken) => {
            ...
            const supplyMantissaData: RewardsDataForMantissa = {
              ...
              underlyingTotalSupply: cTokenData.totalSupply.toNumber(),
...
const constructMantissa = (
  ...
  return (rewardSpeed * rewardEthPrice) / ((underlyingTotalSupply * underlyingEthPrice) / 1e18);
};
    ...
    async () => {
      ...
      await Promise.all(
        cTokenAddrs.map(async (cTokenAddr) => {
          ...
          await ctokenInstance.methods.decimals().call();
          const cTokenTotalSupply = await ctokenInstance.methods.totalSupply().call();

          const exchangeRateCurrent = await ctokenInstance.methods.exchangeRateCurrent().call();
          ...
          const obj: CTokenDataForRewards = {
            ...
            cToken: cTokenAddr,
            totalSupply: BigNumber.from(underlyingTotalSupply2) ?? BigNumber.from(0),
          };
...
export const getPriceFromOracles = async (
  ...
  const oracleAddress: string = await comptrollerInstance.methods.oracle().call();
...
export const useAssetPricesInEth = (
  ...
  const { data } = useQuery(
    'asset prices for ' + tokenAddresses.join(',') + ' ' + Object.keys(tokensData).join(','),
    async () => {
