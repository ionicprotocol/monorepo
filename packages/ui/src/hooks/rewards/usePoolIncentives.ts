...
export function usePoolIncentives(comptroller?: string): IncentivesData {
  ...
  const { data } = useQuery('Pool incentives for comptroller ' + comptroller, async () => {
...
export const useCTokensUnderlying = (cTokenAddrs: string[]): CTokensUnderlyingMap => {
  const { fuse } = useRari();
    ...
    async () => {
      ...
      await Promise.all(
        cTokenAddrs.map(async (cTokenAddr) => {
          ...
          _map[cTokenAddr] = await ctokenInstance.callStatic.underlying();
        })
      );
