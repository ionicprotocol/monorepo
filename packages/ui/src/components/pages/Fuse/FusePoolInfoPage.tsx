...
export const PoolInfoBox = ({ data }: { data: ReturnType<typeof useFusePoolData> }) => {
      ...
      >
        {data ? (
          data.assets.length > 0 ? (
            ...
            <Center>{'There are no assets in this pool.'}</Center>
          )
        ) : (
          <Column
            mainAxisAlignment="flex-start"
...
const OracleAndInterestRates = ({ poolData }: { poolData: ReturnType<typeof useFusePoolData> }) => {
...
const StatRow = ({
  ...
  return (
    <Tr borderTopWidth={'1px'} borderColor={cardDividerColor} {...tableRowProps}>
      <Td
...
const AssetOption = ({ asset }: { asset: USDPricedFuseAsset }) => {
  ...
  return (
    <option value={asset.cToken} key={asset.cToken} style={{ color: textColor }}>
    ...
    >
      ...
      >
        ...
        <Select
          ...
          _focus={{ outline: 'none' }}
          onChange={(event) =>
            setSelectedAsset(assets.find((asset) => asset.cToken === event.target.value)!)
          }
