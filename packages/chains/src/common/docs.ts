export const defaultDocs = (blockExplorerUrl: string, tokenAddress: string): string => {
  ...
  <a href="${blockExplorerUrl}/token/${tokenAddress}" target="_blank" style="color: #BCAC83">Official Block Explorer</a>, where
...
export const ellipsisDocs = (poolAddress: string, poolName: string, tokenAddress: string) => {
  return `Head over to the <a href="https://ellipsis.finance//${poolAddress}" target="_blank" style="color: #BCAC83">${poolName} Ellipsis Finance Pool</a>
  and click on "Add Liquidity". You can then supply any of the underlying assets, and upon adding liquidity, you 
  will get back the <a href="https://bscscan.com/address/${tokenAddress}" target="_blank" style="color: #BCAC83">${poolName} pool LP tokens</a>. Come back 
  back here and hit "MAX" to deposit them all in this pool. 
  `;
};

export const ankrBNBDocs = (variant: string) => {
  return `Head over to <a href="https://www.ankr.com/staking/stake/bnb/?token=${variant}" target="_blank" style="color: #BCAC83">Ankr BNB Staking</a>, where you
...
export const pcsDocs = (token0: string, token1: string, poolName: string, tokenAddress: string) => {
  return `<p>You can acquire these Pancakeswap LP tokens <a href="https://pancakeswap.finance/add/${token0}/${token1}" target="_blank" style="color: #BCAC83">here</a>.
  ...
  You will get back <a href="https://bscscan.com/address/${tokenAddress}" target="_blank" style="color: #BCAC83">Pancakeswap ${poolName} LP tokens</a> in your wallet. 
