...
task("deploy:mode:ezeth:irm").setAction(async ({}, { run, ethers }) => {
  ...
  const drm15 = await ethers.getContract("DiscouragingJumpRateModel_15");

  const cToken = sdk.createICErc20(ezEthMarket, deployer);
  const ptx = await cToken._setInterestRateModel(drm15.address);
  console.log("ptx: ", ptx);
...
task("deploy:discouraging:irm").setAction(async ({}, { ethers, deployments, getChainId, getNamedAccounts }) => {
  ...
  const jrm = await deployments.deploy("DiscouragingJumpRateModel_15", {
    contract: "JumpRateModel",
    from: deployer,
    args: [
      config.blocksPerYear,
      ethers.utils.parseEther("0.15").toString(), // baseRatePerYear    15% // too high in order to discourage borrowing
