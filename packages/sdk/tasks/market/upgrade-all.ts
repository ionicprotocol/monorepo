...
task("market:set-latest", "Updates the markets' implementations whitelist").setAction(async (taskArgs, { ethers }) => {
  ...
  if (latestCErc20Delegate === constants.AddressZero || latestCErc20Delegate !== erc20Delegate.address) {
    tx = await fuseFeeDistributor._setLatestCErc20Delegate(1, erc20Delegate.address, "0x00");
  ...
  if (
    ...
    tx = await fuseFeeDistributor._setLatestCErc20Delegate(2, erc20PluginDelegate.address, becomeImplementationData);
