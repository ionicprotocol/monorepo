...
task("liquidate", "Liquidate a position without a flash loan")
  .addParam("borrower", "The address of the borrower whose debt will be repaid and collateral will be seized", undefined, types.string, false)
  ...
  .addOptionalParam("minOutputAmount", "The minimum amount of collateral to seize (or the minimum exchange output if applicable) required for execution. Reverts if this condition is not met.", 0, types.int)
  .addOptionalParam("exchangeSeizedTo", "If set to an address other than `cTokenCollateral`, exchange seized collateral to this ERC20 token contract address (or the zero address for NATIVE).", "", types.string)
  .addParam("uniswapV2Router", "The UniswapV2Router to use to convert the seized underlying collateral. (Is interchangable with any UniV2 forks)")
  .addParam("redemptionStrategies", "The IRedemptionStrategy contracts to use, if any, to redeem 'special' collateral tokens (before swapping the output for borrowed tokens to be repaid via Uniswap).", [], types.json)
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    await fuseSafeLiquidator["safeLiquidate(address,uint256,address,address,uint256,address,address,address[],bytes[])"](
      taskArgs.borrower,
