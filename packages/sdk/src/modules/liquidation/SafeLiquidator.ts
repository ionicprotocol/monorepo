...
export function withSafeLiquidator<TBase extends CreateContractsModule>(
  ...
  return class SafeLiquidator extends Base {
    ...
    async getPotentialLiquidations<T extends LiquidatablePool | PythLiquidatablePool>(
      ...
      // Determine maxHealthFactor based on botType
      let maxHealthFactor: bigint;
      if (botType === BotType.Pyth) {
        maxHealthFactor = await this.contracts.IonicLiquidator.read.healthFactorThreshold();
      } else {
        maxHealthFactor = parseEther("1");
      }
