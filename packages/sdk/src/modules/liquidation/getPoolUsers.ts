...
async function getFusePoolUsers(
  ...
  for (const user of users) {
    const assets = await sdk.contracts.PoolLens.simulate.getPoolAssetsWithData([comptrollerInstance.address], {
      account: user
    });

    const { totalBorrow, totalCollateral } = getUserTotals(assets.result as PoolAssetStructOutput[]);
