...
import {deployAssets, setUpLiquidation, setUpPriceOraclePrices, tradeNativeForAsset} from "../utils";
...
(process.env.FORK_CHAIN_ID ? describe.only : describe.skip)("#safeLiquidateWithFlashLoan", () => {
    ...
    beforeEach(async () => {
        poolNames = ["Standard", "LP Pool", "LP With Rewards"];
        ...
        const masterPriceOracle = (await ethers.getContractAt(
            "MasterPriceOracle",
        ...
        const deployedAssets = await deployAssets(assets.assets, deployer);
