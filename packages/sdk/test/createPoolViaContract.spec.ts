...
type FusePoolDirectory = any;
type Comptroller = any;
...
describe("FusePoolDirectory", function () {
  ...
  describe("Deploy pool", async function () {
    it("should deploy the pool via contract", async function () {
      ...
      const { comptroller, name: _unfiliteredName } = await allPools.filter((p: { creator: string; }) => p.creator === alice.address).at(-1);
