...
import { chainIdtoChain, ozeantest } from "@ionicprotocol/chains";
import { COMPTROLLER_MAIN } from ".";

const ozeantestAssets = ozeantest.assets;

task("markets:deploy:ozeantest:new", "deploy new ozeantest assets").setAction(
  async (_, { viem, run, deployments, getNamedAccounts, getChainId }) => {
    ...
    for (const asset of ozeantestAssets.filter((asset) => assetsToDeploy.includes(asset.symbol))) {
...
task("ozeantest:set-caps:new", "one time setup").setAction(
...
task("market:set-cf:ozeantest:new", "Sets CF on a market").setAction(
