...
import { chainIdtoChain, soneium } from "@ionicprotocol/chains";
import { COMPTROLLER_MAIN } from ".";

const soneiumAssets = soneium.assets;

task("markets:deploy:soneium:new", "deploy new soneium assets").setAction(
...
task("soneium:set-caps:new", "one time setup").setAction(
  async (_, { viem, run, getNamedAccounts, deployments, getChainId }) => {
    ...
    for (const asset of soneium.assets.filter((asset) => assetsToDeploy.includes(asset.symbol))) {
      const pool = await viem.getContractAt("IonicComptroller", COMPTROLLER_MAIN, {
        client: { public: publicClient, wallet: walletClient }
      });
      const cToken = await pool.read.cTokensByUnderlying([asset.underlying]);
...
task("market:set-cf:soneium:new", "Sets CF on a market").setAction(
