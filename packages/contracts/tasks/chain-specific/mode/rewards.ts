...
task("mode:add-rewards:epoch6:supply", "add rewards to a market").setAction(
  async (_, { viem, deployments, getNamedAccounts }) => {
    const { deployer, multisig } = await getNamedAccounts();
    const rewardToken = ION;
    const rewardTokenName = "ION";
    ...
    await setupRewards(
      ...
      deployments,
      multisig as Address,
      "IonicFlywheel_ION_epoch6",
      "IonicFlywheelDynamicRewards_ION_epoch6"
    );
  }
);

task("mode:add-rewards:epoch6:borrow", "add rewards to a market").setAction(
  async (_, { viem, deployments, getNamedAccounts }) => {
    const { deployer, multisig } = await getNamedAccounts();
    const rewardToken = ION;
    const rewardTokenName = "ION";
    ...
    await setupRewards(
      ...
      deployments,
      multisig as Address,
      "IonicFlywheelBorrow_Borrow_ION_epoch6",
      "IonicFlywheelDynamicRewards_Borrow_ION_epoch6"
    );
  }
);
