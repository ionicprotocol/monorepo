...
contract ezEthWethLeveredPositionTest is Test {
  ...
  function test_ezEthWeth() public {
    ...
    ILiquidatorsRegistry registry = ILiquidatorsRegistry(0xc71B968C6C23e2723Bae32957D815C9bE3ca1b34);
    IRedemptionStrategy redemptionStrategy = IRedemptionStrategy(0x83fa5224C7218a731DB18E26ad6Df47dcf7fB73d); // new univ3liquidatorfunder
    vm.startPrank(registry.owner());
    registry._setRedemptionStrategy(
      redemptionStrategy,
      IERC20Upgradeable(address(collateralMarket.underlying())),
      IERC20Upgradeable(address(stableMarket.underlying()))
    );
    registry._setRedemptionStrategy(
      redemptionStrategy,
      IERC20Upgradeable(address(stableMarket.underlying())),
