...
contract veION is Ownable2StepUpgradeable, ERC721Upgradeable, ReentrancyGuardUpgradeable, IveION {
  ...
  mapping(address => LpTokenType) public s_addressToLpType;
  /// @dev Maps LP token types to their corresponding addresses.
  mapping(LpTokenType => address) public s_lpTypeToAddress;
  ...
  function createLockFor(
    ...
    return _createLock(_lpTypes, _tokenAmount, _duration, _stakeUnderlying, _to);
  }
  ...
  function createLock(
    ...
    return _createLock(_lpTypes, _tokenAmount, _duration, _stakeUnderlying, msg.sender);
  }
  ...
  function lockAdditionalAsset(
    ...
    address _tokenAddress = s_lpTypeToAddress[_lpType];
    LockedBalance storage lockedBalance = s_locked[_tokenId][_lpType];
    ...
    if (_tokenAmount < s_minimumLockAmount[_lpType]) revert MinimumNotMet();
    ...
    if (lockedBalance.isPermanent) s_permanentLockBalance[_lpType] += _tokenAmount;
  ...
  /// @inheritdoc IveION
  function increaseUnlockTime(LpTokenType _lpType, uint256 _tokenId, uint256 _lockDuration) external nonReentrant {
    LockedBalance memory oldLocked = s_locked[_tokenId][_lpType];
    ...
    _depositFor(_lpType, _tokenId, 0, unlockTime, false, oldLocked, DepositType.INCREASE_UNLOCK_TIME, _msgSender());
  }

  /// @inheritdoc IveION
  function withdraw(LpTokenType _lpType, uint256 _tokenId) external nonReentrant {
    ...
    if (s_voted[_tokenId]) revert AlreadyVoted();
    if (oldLocked.isPermanent) revert PermanentLock();
    if (_tokenAddress == address(0)) revert TokenNotWhitelisted();
    ...
    if (block.timestamp < oldLocked.end) {
      WithdrawalFactors memory factors;
      factors.daysLocked = ((oldLocked.end - oldLocked.start) * 1e18) / 1 days;
      factors.daysLeft = ((oldLocked.end - block.timestamp) * 1e18) / 1 days;
      factors.timeFactor = (factors.daysLeft * 1e18) / factors.daysLocked;
      factors.veLPLocked = s_supply[_lpType];
      factors.LPInCirculation = IERC20(_tokenAddress).totalSupply();
      factors.ratioFactor = 1e18 - (factors.veLPLocked * 1e18) / factors.LPInCirculation;
      fee = (factors.timeFactor * factors.ratioFactor * oldLocked.boost) / 1e36;
    ...
    for (uint256 i = 0; i < lockedAssets.length; i++) {
      LpTokenType assetLpType = s_addressToLpType[lockedAssets[i]];
  ...
  function merge(uint256 _from, uint256 _to) external nonReentrant {
    ...
    for (uint256 i = 0; i < assetsLocked.length; i++) {
      ...
      LpTokenType lpType = s_addressToLpType[asset];
  ...
  /// @inheritdoc IveION
  function lockPermanent(LpTokenType _lpType, uint256 _tokenId) external nonReentrant {
    address _tokenAddress = s_lpTypeToAddress[_lpType];
  ...
  /// @inheritdoc IveION
  function unlockPermanent(LpTokenType _lpType, uint256 _tokenId) external nonReentrant {
    address _tokenAddress = s_lpTypeToAddress[_lpType];
    LockedBalance memory _newLocked = s_locked[_tokenId][_lpType];
    if (ownerOf(_tokenId) != _msgSender()) revert NotOwner();
  ...
  /// @inheritdoc IveION
  function delegate(uint256 fromTokenId, uint256 toTokenId, LpTokenType lpType, uint256 amount) external nonReentrant {
    address lpToken = s_lpTypeToAddress[lpType];
   ...
   * @param lpType Type of LP token associated with the delegation.
   * @param amount Amount of delegation to remove.
   */
  function _removeDelegation(uint256 fromTokenId, uint256 toTokenId, LpTokenType lpType, uint256 amount) internal {
    address lpToken = s_lpTypeToAddress[lpType];
    LockedBalance memory fromLocked = s_locked[fromTokenId][lpType];
    LockedBalance memory toLocked = s_locked[toTokenId][lpType];
  ...
  function removeDelegatees(
    ...
    address _tokenAddress = s_lpTypeToAddress[lpType];
    if (toTokenIds.length != amounts.length) revert ArrayMismatch();
    if (_tokenAddress == address(0)) revert TokenNotWhitelisted();

    for (uint256 i = 0; i < toTokenIds.length; i++) {
      _removeDelegation(fromTokenId, toTokenIds[i], lpType, amounts[i]);
    }
  }
  ...
  function removeDelegators(
    ...
    for (uint256 i = 0; i < fromTokenIds.length; i++) {
      _removeDelegation(fromTokenIds[i], toTokenId, lpType, amounts[i]);
    }
  }

  /// @inheritdoc IveION
  function claimEmissions(LpTokenType _lpType) external nonReentrant {
    address _tokenAddress = s_lpTypeToAddress[_lpType];
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
    ...
    if (from != address(0) && to != address(0)) {
      ...
      for (uint256 i = 0; i < assetsLocked.length; i++) {
        address asset = assetsLocked[i];
        LpTokenType _lpType = s_addressToLpType[asset];
        ...
        if (delegatees.length != 0) {
          removeDelegatees(tokenId, delegatees, _lpType, amounts);
        }
  ...
  function withdrawProtocolFees(address _tokenAddress, address _recipient) external onlyOwner {
    LpTokenType lpType = s_addressToLpType[_tokenAddress];
  ...
  function _depositFor(
    ...
    vars.tokenAddress = s_lpTypeToAddress[_lpType];
    if (vars.tokenAddress == address(0)) revert TokenNotWhitelisted();
    ...
    vars.newLocked.tokenAddress = vars.tokenAddress;
    ...
    if (_tokenAmount != 0) {
      s_userCumulativeAssetValues[ownerOf(_tokenId)][vars.tokenAddress] += _tokenAmount;
      IERC20(vars.tokenAddress).safeTransferFrom(vars.from, address(this), _tokenAmount);
      ...
      if (address(_stakeStrategy) != address(0) && _stakeUnderlying) {
        _handleTokenStake(_to, _tokenId, vars.tokenAddress, _tokenAmount, _stakeStrategy, _stakeData);
      }
    }
   ...
   * @param _lpTypes Array of token addresses to lock.
  ...
  function _createLock(
    ...
    uint256 _length = _lpTypes.length;
    _mint(_to, _tokenId);

    if (
      _lpTypes.length != _tokenAmount.length ||
    ...
    for (uint i = 0; i < _length; i++) {
      LpTokenType _lpType = _lpTypes[i];
      address _tokenAddress = s_lpTypeToAddress[_lpType];
      uint256 _unlockTime = ((block.timestamp + _duration[i]) / WEEK) * WEEK;
  ...
  /// @inheritdoc IveION
  function setLpTokenType(address _tokenAddress, LpTokenType _lpType) external onlyOwner {
    require(_tokenAddress != address(0), "Invalid token address");
    s_addressToLpType[_tokenAddress] = _lpType;
    s_lpTypeToAddress[_lpType] = _tokenAddress;
    emit LpTokenTypeSet(_tokenAddress, _lpType);
  }
