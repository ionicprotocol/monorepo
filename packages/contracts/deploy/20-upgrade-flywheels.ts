...
const func: DeployFunction = async ({ run, viem, getNamedAccounts, deployments }) => {
  ...
  for (const pool of pools) {
    ...
    for (const ionicFlywheelAddress of flywheels) {
      ...
      const flywheelUpgradeTx = await proxyAdmin.write.upgrade([ionicFlywheelAddress, newImplementationReceipt.address]);

      if (flywheelUpgradeTx)
        await publicClient.waitForTransactionReceipt({ hash: flywheelUpgradeTx as Address });
